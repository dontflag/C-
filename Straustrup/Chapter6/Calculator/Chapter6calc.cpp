...
double prim(bool get) // обработка первичных выражений 
{
	...
	case NAME:
	{
		...
		double &v = table[rec.string_value];
...
double func()
{
string func_name = rec.string_value;
...
vector<pair<string,double>> params; //список параметр = значение (if any)
	...
	string func_def="(";
		do
		{
			...
			{
			case NAME:
				func_def += rec.string_value;
				break;
			case NUMBER:
				func_def += to_string(rec.number_value);
				break;
			case END:
	...
	string param_name;
		for (short i = 1; funcs[func_name][i]!=')'; i++) //1, чтобы пропустить '('
		{
			...
			if (funcs[func_name][i] == ',')
			{
				...
				params.push_back(std::make_pair(param_name, rec.number_value));
				param_name = "";
